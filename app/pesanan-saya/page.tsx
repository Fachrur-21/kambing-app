...
type OrderRecord = {
  ...
  buyerName: string
  total: number
  status: 'diproses' | 'dikirim' | 'selesai' | string
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!meResponse.ok || userRole !== 'pembeli') {
        ...
        const orderResponse = await fetch('/api/pesanan', {
          headers: { Authorization: `Bearer ${storedToken}` }
        })
        const orderResult: OrderResponse = await orderResponse.json()

        if (!orderResponse.ok) {
          setMessage(orderResult?.message || 'Gagal memuat daftar pesanan')
          setOrders([])
          return
        }
        ...
        setMessage('Terjadi error saat memuat daftar pesanan')
      } finally {
        setLoading(false)
      }
    }
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <article className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{order.buyerName || nama}</p>
                ...
                <article key={item.id} className="flex gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-4 text-xl text-slate-700">
                      ...
                      <p className="font-black text-emerald-700">{formatRupiah(item.harga || item.subtotal)}</p>
                    </div>
                  </div>
                </article>
              ))}
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f5f8]">
        <div className="text-center">
          ...
          <p className="text-slate-700">Memuat pesanan saya...</p>
        </div>
      </main>
    )
  }
    ...
    <main className="flex min-h-screen bg-[#edeff4] text-slate-800">
      ...
      <section className="flex-1 p-4 lg:p-6">
        <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h1 className="text-4xl font-black tracking-tight text-slate-900">Pesanan Saya</h1>
          <p className="mt-2 text-lg text-slate-600">Pantau status dan detail pengiriman semua pesanan Anda</p>
        </div>
